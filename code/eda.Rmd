...
```{r select}
data <- select(source_data, dteday, hr, holiday, temp, atemp, hum, windspeed, cnt)
```
...
```{r dummy}
data <- dummy_cols(data, select_columns = c("hr_bin"), remove_first_dummy = TRUE)
head(data, 10)
```