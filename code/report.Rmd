...
Create clusters for `hr`  called `hr_seg`. 24 dummy codes it too much.
```{r hour cut}
...
model2        <- lm(cnt ~ temp + hum, data)
model3        <- lm(cnt ~ temp + hum + weathersit, data)
model4        <- lm(cnt ~ temp + hum + weathersit + windspeed, data)

# Include moderator for model of choice (#2), for comparison analysis.
model2_mod    <- lm(cnt ~ (temp + hum)* hr_seg, data)
```
...
comp_df <- data.frame(
  ...
  predictors = c('temp', 'temp + hum', 'temp + hum + weathersit', 'temp + hum + weathersit + windspeed', 'temp + hum'),
  moderator = c('', '', '', '', 'hr_seg'),
